...
Prometheus has a single global retention setting and no built-in per-metric retention (https://github.com/prometheus/prometheus/issues/1381[upstream issue]). We want to keep environmental metrics (temperature, humidity, electricity, etc.) for years but only need pi system metrics (`node_*`, etc.) for ~60 days. The workaround is a daily systemd timer that calls Prometheus's `delete_series` + `clean_tombstones` admin APIs to trim series outside the keep-list older than 60 days. See https://github.com/dasl-/pitools/issues/13[issue #13] for the alternatives we considered.

The keep-list is a single regex (`KEEP_REGEX`) in https://github.com/dasl-/pitools/blob/main/observability/prometheus_retention_cron[`prometheus_retention_cron`]. Currently it's `^(ambient_|city_|electricity_).*`, which matches everything emitted by `sensors/measure_temp_and_humidity`, `sensors/measure_air_quality`, `sensors/measure_city_data`, and `sensors/measure_electricity_usage`.

### Adding a new long-retention metric
**If you add a new metric you want retained for years, you must add its prefix to `KEEP_REGEX` before its data ages past 60 days, or the trailing edge will be deleted.** Mitigation: the cron runs a sanity check (`SAFETY_MIN_KEEP_SERIES`) that aborts the whole run if `KEEP_REGEX` matches fewer than the expected number of keep-list series — this protects against typos in the regex itself, not against forgetting to add a new metric.
...
. Confirm the sanity check passes and the reported non-keep-list series count is sane (low thousands, not millions).
...
....
. Watch `du -sh /ssd/prometheus-data` over 60–70 days. The first ~60 days nothing happens (no data is yet old enough). From day 60 the size should plateau and then track only keep-list ingest.

### Snapshot before destructive changes
Before flipping `DRY_RUN=false` for the first time (or before any change to `KEEP_REGEX`), take a Prometheus TSDB snapshot. The snapshot uses hardlinks so it costs near-zero disk at creation, and is a clean atomic point-in-time copy.
....
