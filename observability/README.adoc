...
The keep-list is a single regex (`KEEP_REGEX`) in https://github.com/dasl-/pitools/blob/main/observability/prometheus_retention_timer[`prometheus_retention_timer`]. Currently it's `^(ambient_|city_|electricity_).*`, which matches everything emitted by `sensors/measure_temp_and_humidity`, `sensors/measure_air_quality`, `sensors/measure_city_data`, and `sensors/measure_electricity_usage`.

### Adding a new long-retention metric
**If you add a new metric you want retained for years, you must add its prefix to `KEEP_REGEX` before its data ages past 60 days, or the trailing edge will be deleted.** Mitigation: the timer runs a sanity check (`SAFETY_MIN_KEEP_SERIES`) that aborts the whole run if `KEEP_REGEX` matches fewer than the expected number of keep-list series — this protects against typos in the regex itself, not against forgetting to add a new metric.

### Install
....
ssh pi@study.local 'cd /home/pi/development/pitools && git pull && /home/pi/development/pitools/observability/install_prometheus_retention_timer'
....
The installer drops a `prometheus-retention.service` (oneshot) and a `prometheus-retention.timer` (daily 03:00, `Persistent=true`). It defaults to `DRY_RUN=true` so a fresh install is non-destructive.

### Verify and flip DRY_RUN=false
. Trigger a dry run immediately: `sudo systemctl start prometheus-retention.service`
. Tail logs: `sudo journalctl -u prometheus-retention.service -n 50`
...
....
sudo sed -i 's/Environment=DRY_RUN=true/Environment=DRY_RUN=false/' /etc/systemd/system/prometheus-retention.service
sudo systemctl daemon-reload
sudo systemctl start prometheus-retention.service
....
...
....
The snapshot lives on the same `/ssd` as the live TSDB, so this protects against logical errors (bad timer regex, accidental deletion) but **not** against `/ssd` disk failure.
