...
. install timestamped_exporter, grafana, prometheus, and prometheus_retention_timer on one host: study.local
...
Prometheus has a single global retention setting and no built-in per-metric retention (https://github.com/dasl-/pitools/issues/13[issue]). We want to keep environmental metrics (temperature, humidity, electricity, etc.) for years but only need pi system metrics (`node_*`, etc.) for a few months. The workaround is a daily systemd timer that calls Prometheus's `delete_series` + `clean_tombstones` admin APIs to trim series outside the keep-list older than a few months.

The keep-list is a single regex (`KEEP_REGEX`) in https://github.com/dasl-/pitools/blob/main/observability/prometheus_retention_timer[`prometheus_retention_timer`].
